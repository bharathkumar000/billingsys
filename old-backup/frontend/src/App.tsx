...
export default function VoiceBillingApp() {
  ...
  const [terminalView, setTerminalView] = useState<'chat' | 'metadata'>('chat');
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      rec.onerror = (e: any) => {
        ...
        triggerNotification('Microphone error. Check browser permissions.');
      };
      rec.onend = () => {
        setIsRecording(false);
        if (statusMessage === 'LISTENING') setStatusMessage('READY');
      };
      rec.onresult = (e: any) => {
  ...
  const startVoiceRecording = () => {
    ...
    rec.onerror = (err: any) => {
      console.error('Voice Assistant Mic Error:', err);
      setIsVoiceRecording(false);
      triggerNotification('Microphone error or permission denied.');
    };
    rec.onend = () => {
      setIsVoiceRecording(false);
    };
  ...
  const handleToggleMic = () => {
    if (!recognitionRef.current) {
      triggerNotification('Speech recognition requires Chrome or Edge');
      return;
    }
    if (isRecording) {
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <button className="btn btn-ghost" onClick={handleAddRowManually} title="Add raw item row manually">
                  <Plus size={13} />
                  Add Row
                </button>
          ...
          >
            ...
            >
              ...
              >
                ...
                <button
                  style={{
                    ...
                    fontWeight: 600,
                    cursor: 'pointer',
                    background: terminalView === 'metadata' ? 'var(--bg-elevated)' : 'transparent',
                    color: terminalView === 'metadata' ? 'var(--accent-primary)' : 'var(--text-secondary)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    gap: '6px',
                  ...
                  onClick={() => setTerminalView('metadata')}
                >
                  <FileText size={12} style={{ color: terminalView === 'metadata' ? 'var(--accent-primary)' : 'var(--text-dim)' }} />
              ...
              >
                ...
                <span className="font-mono" style={{ fontSize: '9px', color: 'var(--text-secondary)', fontWeight: 600 }}>
                  {listenerLanguage === 'en-IN' ? 'EN' : 'KN'}
                </span>
              </div>
            </div>

            {terminalView === 'metadata' ? (
              <div
                style={{
                  flex: 1,
                  overflowY: 'auto',
