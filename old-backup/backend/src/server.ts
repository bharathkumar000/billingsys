...
import { compileBilingualInvoicePDF } from './pdf';
...
app.get('/api/sessions', async (req: Request, res: Response) => {
  try {
    const sessions = await dbAll('SELECT * FROM billing_sessions ORDER BY created_at DESC');
...
app.post('/api/sessions', async (req: Request, res: Response) => {
  ...
  try {
    await dbRun(
      ...
      [sessionId, sessionTitle, isoDate, isoDate]
    );
...
app.get('/api/sessions/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const history = await dbAll('SELECT * FROM chat_history WHERE session_id = ? ORDER BY created_at ASC', [id]);
...
app.post('/api/sessions/:id/commit', async (req: Request, res: Response) => {
  const { id } = req.params;
  const { 
    items = [], 
    chatLog = [], 
